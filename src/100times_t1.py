...
# 算法变量设置
# 总仿真次数
def SingleSdFth(count=10, x=np.arange(0.55, 0.95, 0.05), topology_fidelity_mode=0, nrof_requests=10000, alg1_mode=1,
                link_capacity=10, random_topology=0, random_topology_mode=0, random_topology_nodes_num=5):
    """
    count=10
    ...
    for i in range(len(x)):
        ...
        if enable_alg1 == 1:
            ...
            #1
            res1.append(stpalg1[i])
            #2
            res1.append(avefalg1[i])
            #3
            res1.append(purround1[i])
            #4
            res1.append(consualg1[i])
            #5
            res1.append(time1[i])
        ...
        if enable_alg2 == 1:
            ...
            # 1
            res1.append(stpalg2[i])
            # 2
            res1.append(avefalg2[i])
            # 3
            res1.append(purround2[i])
            # 4
            res1.append(consualg2[i])
            # 5
            res1.append(time2[i])
...
Res = [ ]
FileRes = " "
Throughput_Qpath = [0,0,0,0,0,0,0,0]
Throughput_Qleap = [0,0,0,0,0,0,0,0]
Fidelity_Qpath = [0,0,0,0,0,0,0,0]
Fidelity_Qleap = [0,0,0,0,0,0,0,0]


fp = open('100_times_FvsT.txt', 'w')

for i in range(10):
    T1 = time.time()
    print(i)
    Res = SingleSdFth(s1_count,s1_x,s1_topology_fidelity_mode,s1_nrof_requests,s1_alg1_mode,s1_link_capacity,s1_random_topology,s1_random_topology_mode,s1_random_topology_nodes_num)
    Result.append(Res)
    FileRes += str(Res) + '\n'
    T2 = time.time()
    print('time: ' % (T2 - T1))
...
fp = open('100_times_FvsT_Result.txt', 'w')

for i in range(10):
    for j in range(8):
        Throughput_Qpath[j] += Result[i][j*10]
        Throughput_Qleap[j] += Result[i][(j*10) + 5]
        Fidelity_Qpath[j] += Result[i][(j*10)+1]
        Fidelity_Qleap[j] += Result[i][(j*10) + 6]
        if not Throughput_Qpath[j] or not Throughput_Qleap[j] or not Fidelity_Qpath[j] or not Fidelity_Qleap[j]:
            flag = 1


Throughput_Qpath = Throughput_Qpath / 10
Throughput_Qleap = Throughput_Qleap / 10
Fidelity_Qpath = Fidelity_Qpath / 10
Fidelity_Qleap = Fidelity_Qleap / 10
fp.write('\n'+str(Throughput_Qpath)+'\n')
fp.write(str(Throughput_Qleap)+'\n')
fp.write(str(Fidelity_Qpath)+'\n')
fp.write(str(Fidelity_Qleap)+'\n')
