...
def main():
    ...
    for i in range(count):
        ...
        while len(route) != 2: # node number (hop number = node num - 1)
            des = random.randint(0, len(g) - 1)
            route = Spfsearch().heapdijkstra(table, sou, des)
    ...
    #
            time_2 = time.time()
            if enable_alg2 == 1:
                path2, d2, fi2, con2, th2, sumt2, times2, num_pud, num_pud0, t_li, n = Qleap().alg2(copy.deepcopy(g), sou, des, x[j], nrof_requests)
