...
class Qleap:
    ...
    def alg2(self,network,source,des,fth,request):
        ...
        while True:
            ...
            print(Pathf()._pathf(g,li), d_li, fave)
            #判断路径上是否有足够资源
            #n=1
            if itera == 0:
                num_pud0 = sum(d_li)
            ...
            if Udtp().preudtppath(g,li,d_li):
                ...
                patht_li=n*t_li
                timecal[0] += (time.time() - time_start)
            ...
            #更新路径容量
            #Udtp().udtppath(g,li,Etp().calactcon(d_li,n),self.fth)
        timecal[1] += (time.time() - time_start)
        #actualpath,pathfi,pathd,pathth,countnz=Patharr().patharr(self.path,self.fi,self.d,self.th)
        return self.path,self.d,self.fi,self.con,self.th,self.sumt,times, num_pud, num_pud0, t_li, n, timecal
